...
has globally Lipschitz coefficients and at most linear growth.  In this paper
\(Y_t=I_t\), the imbalance state, and \(R_t\) is present only in
Section~\ref{sec:regime}.  A pure
...
\(O(r_{\rm G})\) and the inner shift \(s/3\) is \(O(r_{\rm G}^2)\).  Both
are dropped, and because the prefactor multiplies the whole execution sum the
cost is a \emph{relative} error:
\begin{equation}
\frac{\Lambda^{a,0}}{k}\chi_\gamma\e^{k(v_{q-1}-v_q)}
+\frac{\Lambda^{b,0}}{k}\chi_\gamma\e^{k(v_{q+1}-v_q)}
=\frac{2c_{\rm GLFT}}{k\cosh i}\,
\cosh\bigl\{\vartheta+rq-s q^2\bigr\}\bigl(1+O(r_{\rm G})\bigr).
\label{eq:toy_cubic_cosh}
\end{equation}
Every coefficient matched below therefore carries a relative
\(1+O(r_{\rm G})\) error, about \(2.6\%\) at the baseline.  The distinction
matters: an additive \(O(r_{\rm G})\) would be of size
\(\tfrac{2c_{\rm GLFT}}{k}\tfrac{r_{\rm G}}{2}\approx8.6\times10^{-3}\),
an order of magnitude above the inventory-risk coefficient
\(a_{\rm c}/k=4.5\times10^{-4}\) that the \(q^2\) balance matches, and could
not support the matching that follows.
...
slope term \(E_i'(0)\,p\) splits between two orders: it feeds the \(q^1\)
balance \eqref{eq:toy_q1_match} through \(r\sinh\vartheta\) and, one order
down, contributes the slope--cubic cross term \(-s\sinh\vartheta\,q^2\) to
the \(q^2\) balance \eqref{eq:toy_cubic_q2}.  What sets the inventory curvature is how
\(E_i\) curves with \(p\).  The inventory-risk contribution enters the balance
as \(-\tfrac{a_{\rm c}}{k}q^2\), so we match the nonlinear part
\(E_i(p)-E_i(0)-E_i'(0)p\) to \(\tfrac{a_{\rm c}}{k}q^2\), deferring the
slope--cubic cross term to the residual \(\mathcal R_2\) below.  With
...
\begin{table}[htbp]
\centering
\caption{Model, simulation, and diagnostic parameters used in the reported
experiments.}
...
\(c_T\) & terminal liquidation charge & \(0.01\)\\
...
width requires it.  Two drawing conventions run through the figures: in the
depth comparisons the section's numerical RHJB is solid black, the body closed
form dashed orange and the appendix Galerkin form dotted purple; in the
policy-comparison figures each policy keeps one colour throughout.}
\label{tab:policies}
\begin{tabular}{llll}
\toprule
...
and \eqref{eq:lambda_ask}--\eqref{eq:lambda_bid} give
...
This is what makes the inventory distribution of
Figure~\ref{fig:lifetime_inventory_distributions} bimodal: a signal that is
rarely neutral produces a position that is rarely flat.  The maker wants to be
long when buyers dominate and short when sellers dominate, and at the
strong-signal calibration of Table~\ref{tab:parameters} the flow is seldom
balanced.  The target is the myopic mean--variance position---the maximizer of
\(q\alphabar-\tfrac{\gamma\sigma^2}{2}q^2\), which under CARA does not depend on
wealth---and it cannot express more than a direction, because the buy share
\(\tfrac12(1+\tanh i)\) lies in \((0,1)\): the net flow generating the drift,
\(\epsilon(\Lambda^{a,0}-\Lambda^{b,0})=2\bar\Lambda\epsilon\tanh i\), cannot
exceed the total flow \(2\bar\Lambda\epsilon\).  Quantitatively, \(|I_t|>1\)
about \(62\%\) of the time and there \(q^\star_{\rm d}\) already exceeds three
quarters of its ceiling, so its law is itself bimodal (modes \(\pm39.5\),
\(4\%\) of peak at the origin) and the realized inventory inherits that shape.
The \(I\)-aware RHJB is the control that rules out the simpler reading that the
alpha policy merely carries more: it reaches a comparable
\(\E[Q_t\mid I_t]=\pm8.3\) and is still unimodal, because it leans
\emph{against} the flow rather than into it and its response saturates more
slowly (\(\E[Q_t\mid I_t=2]/\E[Q_t\mid I_t=4]=0.70\) against \(0.85\)), leaving
\(0.82\) of peak density at the origin against \(0.18\).  What the drift reward
changes is not how much inventory the maker holds but whether it is aiming at a
position or resisting one.  The split is real but not dramatic: between- over
within-signal standard deviation is \(1.16\) for the Alpha/\(I\)-aware policy
and \(0.94\) for the regime cubic, so the conditional spread nearly washes the
two humps out.
\end{remark}
...
Let \(\mathcal R\) be a finite set of regime labels.  Throughout
Sections~\ref{sec:regime}--\ref{sec:numerical_implementation} and
Appendix~\ref{app:regime_refined} the regime label is \(r\), carried as a
subscript on every regime-dependent object (\(z_r\), \(u_{q,r}\),
\(h_{{\rm shift},r}\), \(\vartheta_{{\rm full},r}\), \(s_r\)) and as the last
argument of the quote functions; the inventory curvature is a different object,
written \(r(i)\) when state-dependent and \(r_{\rm G}\) for its frozen GLFT
value, and never carries a regime subscript.  The regime process
...
\eqref{eq:lambda_ask}--\eqref{eq:lambda_bid}, functions of the current
...
\emph{fast} channels---the only change is the regime level \(A_Rz_r\),
carried ungated in the shift,
\begin{equation}
\boxed{\,h_{{\rm shift},r}(i)=\mathcal M_I\,h_I
+\bigl[1+g(\mathcal M_I-1)\bigr]h_\alpha+A_R z_r\,}
\label{eq:regime_sc_shift}
\end{equation}
with \(\vartheta_{{\rm full},r}(i)=i-k\,h_{{\rm shift},r}(i)\) and cubic
\(s_r(i)=\tfrac{r(i)^2}{6}\tanh\vartheta_{{\rm full},r}(i)\) as in
\eqref{eq:fa_selfcons_cubic}, so that
\begin{equation}
\boxed{\;
\begin{aligned}
\delta_{R,\,\text{hybrid}}^a(q,i,r)&=\delta_\gamma+h_{{\rm shift},r}-\tfrac{r(i)}{2k}(2q-1)+\tfrac{s_r(i)}{k}\bigl(q^2-q+\tfrac13\bigr),\\[2pt]
\delta_{R,\,\text{hybrid}}^b(q,i,r)&=\delta_\gamma-h_{{\rm shift},r}+\tfrac{r(i)}{2k}(2q+1)-\tfrac{s_r(i)}{k}\bigl(q^2+q+\tfrac13\bigr),
\end{aligned}\;}
...
The split use of \(\vartheta_I\) in the curvature \(r(i)\) and of
\(\vartheta_{{\rm full},r}\) in the cubic \(s_r\) is a hybrid empirical
closure validated numerically, not a theorem or a
...
but the regime channel does not: \(h_{{\rm shift},r}(0)=A_R z_r\), so a non-neutral
regime keeps the book biased and \(s_r(0)\neq0\); only the fully neutral state
(\(i=0,\ z_r=0\)) recovers the signal-blind GLFT quotes.
...
cubic, whose cubic skew also reads the regime through
\(\vartheta_{{\rm full},r}\), stays closer to the RHJB.}
...
regime: \(\Delta\)penalized PnL against the regime-aware RHJB.  Bars are
\(95\%\) intervals.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{terminal_pnl_distributions.png}
\caption{Terminal-PnL distributions on the shared environment paths
(\(20000\) paths; density on a log scale), each study's policies against the
signal-blind benchmark.  Dotted vertical lines mark the certainty equivalent
\eqref{eq:certainty_equivalent}.  The toy panel is the picture behind the
certainty-equivalent column of Table~\ref{tab:toy_summary}: the benchmark earns
the \emph{higher} mean PnL, \(2408\) against \(2097\), but with \(2.6\) times
the dispersion and a first percentile at \(1037\) against \(1576\), so its
certainty equivalent collapses to \(953\) while the \(I\)-aware RHJB keeps
\(1842\).  The certainty-equivalent gap is therefore a tail effect, not a mean
effect.  Once flow predicts returns the benchmark separates from the rest: it
loses money on \(99.6\%\) of the fast-alpha paths, and in the regime study its
certainty equivalent falls to \(-5476\).}
\label{fig:terminal_pnl_distributions}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{lifetime_inventory_distributions.png}
\caption{Inventory over the simulated lifetime, pooled over paths and times:
the \(\E|Q_t|\) and \(Q^{\rm life}_{.01},Q^{\rm life}_{.99}\) columns of
Tables~\ref{tab:toy_summary}, \ref{tab:alpha_summary}
and~\ref{tab:regime_summary} as full distributions.  The signal-blind
benchmark is the broad, flat hump spanning \(\pm45\), the only policy that
approaches the \(\pm60\) grid bound, while every state-dependent policy
concentrates near zero.  The drift reward is what splits the distribution: in
the martingale toy model all four densities peak at \(q=0\), whereas once
\(q\alphabar(i)\) enters the alpha-using policies turn \emph{bimodal}, with
modes near \(\pm8\) for the Alpha/\(I\)-aware RHJB and \(\pm10\) to
\(\pm11\) for the body cubic and a dip of \(4\%\)--\(26\%\) at the origin.
That is the deliberate directional inventory of
Remark~\ref{rem:why_carry_inventory} seen as a distribution rather than as the
single number \(\E|Q_t|\), which widens from \(5.5\) to \(7.2\) in the
fast-alpha study and from \(5.7\) to \(10.8\) in the regime study.  The origin
is transit rather than a resting point: turning the book from \(+8\) to
\(-8\) means \(16\) units at a realized \(\approx0.37\) fills per unit time,
of the same order as the imbalance half-life \(\log2/\beta\approx55\), and the
Alpha/\(I\)-aware policy accordingly spends \(\Prob(|Q_t|\le2)=0.160\) of its
life near flat against \(0.503\) at the modes.  A single hump at zero says
inventory is a cost to be reverted; two humps say it is a position, and flat is
the least attractive place to be.}
\label{fig:lifetime_inventory_distributions}
\end{figure}
...
(\(\approx2283\)), the ordering of the depth figures.  Paired on the shared
paths these are \(-23.2\pm1.1\) and \(-140.3\pm1.2\), statistically clear
shortfalls rather than ties.  Both forms quote more aggressively into the
...
collects the paired contrasts of the three studies, and
Figures~\ref{fig:terminal_pnl_distributions}
and~\ref{fig:lifetime_inventory_distributions} show the terminal-PnL and
lifetime-inventory distributions behind the table columns.
...
The reported path diagnostics---PnL and its components, inventory, and the
state variables---are sampled into the recorded series every
\(\Delta t_{\rm rec}=1\) time unit rather than at every simulation step; the
off-grid shares below are instead accumulated at every step.  Within each
step, quotes and intensities use the left-endpoint state.  \(S\),
...
Galerkin form use the true \(I_t\).  The shares of simulated time--path
...
the regime model and adds the slow regime channel.  The subscript convention of
Section~\ref{sec:regime} applies: \(r\in\mathcal R\) is the regime label and
\(r(i)\) the curvature.  Under the separable closure
\(h_r(i)=h_G(i)+Bz_r\) the extra regime term in the \(q^1\) balance,
\(z_r[\beta h_G'(i)-(\lambda_R+2\kappa)B]\), is even in \(i\) (\(h_G\) odd),
...
invariance of the full \((i,r)\)-Galerkin residual (it fails if
\(Bz_r\) enters the nonlinear curvature, under the true joint weight
\(p_r(i)\), or with even test functions).  Parity also leaves \(B\)
...
Galerkin-local value rather than at \(A_R\).  No full \((i,r)\)-Galerkin residual is re-solved: the
...
h_{G,r}^{\rm reg}(i)=h_G^{\rm fa}(i)+A_R z_r,\qquad
...
\begin{equation}
\delta_{R,\rm gal}^{a,b}(q,i,r)=\delta_\gamma\pm h_{G,r}^{\rm reg}(i)
\mp\tfrac{r(i)}{2k}(2q\mp1)\pm\tfrac{s(i)}{k}\bigl(q^2\mp q+\tfrac13\bigr),
\qquad r\in\mathcal R.\label{eq:regime_galerkin_quotes}
\end{equation}
...
\(r(i)/r_{\rm G}\approx9\) at \(i=6\).  The verdict, however, does not.  On the
...
\((i,r)\)-slice and on the inventory window.  Unlike its fast-alpha
