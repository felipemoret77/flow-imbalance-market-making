...
\caption{Inventory over the simulated lifetime, pooled over paths and times, on
a log frequency scale: the \(\E|Q_t|\) and \(Q^{\rm life}_{.01},Q^{\rm
life}_{.99}\) columns of Tables~\ref{tab:toy_summary},
\ref{tab:alpha_summary} and~\ref{tab:regime_summary} as full distributions.
The signal-blind benchmark spreads almost flat across \(\pm40\) and is the only
policy that approaches the \(\pm60\) grid bound, while every state-dependent
policy concentrates near zero.  Within the regime study the progressive
widening from the \(I\)-aware RHJB (\(\E|Q_t|=5.7\)) through the
Alpha/\(I\)-aware RHJB (\(7.4\)) to the regime-aware RHJB (\(9.7\)) and the two
closed forms (\(10.8\)) is the deliberate directional inventory of
Remark~\ref{rem:why_carry_inventory}; the same widening, from \(5.5\) to
\(7.2\), follows the drift reward in the fast-alpha panel.}
