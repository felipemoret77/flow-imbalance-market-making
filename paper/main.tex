...
That target is the myopic mean--variance position for a price with drift
\(\alphabar\) and volatility \(\sigma\)---the maximizer of
\(q\alphabar-\tfrac{\gamma\sigma^2}{2}q^2\), which under CARA does not depend on
wealth---so the drift reward makes the book a portfolio rather than an
inventory to be cleared.  It is bounded for an economic reason: the buy share \(\tfrac12(1+\tanh i)\) lies in \((0,1)\), so
the net flow generating the drift,
\(\epsilon(\Lambda^{a,0}-\Lambda^{b,0})=2\bar\Lambda\epsilon\tanh i\), cannot
exceed the total flow \(2\bar\Lambda\epsilon\).  Past a modest imbalance the
signal therefore says \emph{which way} to lean, not how much; since
\(|I_t|>1\) about \(62\%\) of the time, the law of \(q^\star_{\rm d}\) is itself
bimodal (modes \(\pm39.5\), \(4\%\) of peak at the origin), a shape the
realized inventory inherits
(Figure~\ref{fig:lifetime_inventory_distributions}).  What separates the
policies is the shape of the lean, not its size: the \(I\)-aware RHJB reaches a
comparable \(\E[Q_t\mid I_t]=\pm8.3\), but leans \emph{against} the flow and
saturates more slowly (\(\E[Q_t\mid I_t=2]/\E[Q_t\mid I_t=4]=0.70\) against
\(0.85\)), so its mixture over stationary \(I\) keeps \(0.82\) of peak density
at the origin and stays unimodal, against \(0.18\) with modes at \(\pm7.7\).
The split is real but not dramatic: between- over within-signal standard
deviation is \(1.16\) for the Alpha/\(I\)-aware policy and \(0.94\) for the
regime cubic, so the conditional spread nearly washes the two humps out.
\end{remark}
