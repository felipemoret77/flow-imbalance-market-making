...
This is why the inventory distribution of
Figure~\ref{fig:lifetime_inventory_distributions} is bimodal: a signal that is
rarely neutral produces a position that is rarely flat.  The buy share
\(\tfrac12(1+\tanh i)\) lies in \((0,1)\), so the drift---and with it the
target---saturates, and past a modest imbalance the flow says which way to lean
and not how much; the imbalance sits outside the balanced band \(62\%\) of the
time.  The \(I\)-aware RHJB rules out the simpler reading that the alpha policy
merely carries more: it reaches a comparable \(\E[Q_t\mid I_t]=\pm8.3\) and
stays unimodal, because it leans \emph{against} the flow rather than into it.
What the drift reward changes is not how much inventory the maker holds but
whether it is aiming at a position or resisting one.
\end{remark}
...
modes near \(\pm8\) to \(\pm11\) and a shallow dip at the origin
(Remark~\ref{rem:why_carry_inventory}).  The origin is transit rather than a
resting point, and turning the book over takes of the order of the imbalance
half-life, which is why the dip is shallow.  A single hump at zero says
