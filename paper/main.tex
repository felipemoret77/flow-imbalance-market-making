...
\item \textbf{Common-path evaluation.}  We compare the signal-blind GLFT
  ...
  full jump-diffusion simulator.  In penalized PnL the ladder rises by
  \(+4435\), \(+1967\) and \(+175\) (paired s.e.\ \(5.3\), \(4.2\), \(1.8\))
  as flow composition, alpha, and regime information are introduced, and the
  hybrid closed form is statistically indistinguishable from the fast-alpha
  RHJB (\(-0.8\pm1.1\)).
\end{itemize}
...
Lehalle~\cite{GueantLehalle2015}.
...
Cartea, Donnelly, and Jaimungal~\cite{CarteaDonnellyJaimungal2018} quantify how
much order-book signals add to a trading strategy.  The companion execution
...
jump--diffusions gives, for every \(\delta\in\Acal\) and \(t\le T\),
\begin{equation}
g(\Xi_T)=H(t,\Xi_t)
...
growth) has finite polynomial moments of every order \cite{Pham2009}; its
...
exceed \(0.88\) about \(60\%\) of the time, a deliberately strong asymmetry.
...
benchmark, the target the regime closed forms approximate.  In the legend of
Figure~\ref{fig:regime_inventory_by_regime}, ``Regime self-consistent cubic''
denotes the regime hybrid cubic.}
\label{tab:policies}
\begin{tabular}{llll}
\toprule
...
\midrule
Signal-blind Gaussian GLFT & \(2407.65\) & \(953^{\dagger}\) & \(4812.56\) & \(15.53\) & \(-39\) & \(39\)\\
\(I\)-aware RHJB & \(2097.03\) & \(1842\pm6\) & \(3550.00\) & \(5.46\) & \(-13\) & \(14\)\\
\(I\)-aware self-consistent cubic & \(2108.54\) & \(1834\pm12\) & \(3555.50\) & \(5.68\) & \(-13\) & \(13\)\\
\(I\)-aware Galerkin-cubic & \(2095.17\) & \(1836\pm8\) & \(3540.05\) & \(5.55\) & \(-13\) & \(13\)\\
\bottomrule
\end{tabular}
...
shared paths, \(\Delta\mathrm{CE}\) against the RHJB is \(-7.8\) (bootstrap SE
\(10.7\)) for the cubic and \(-5.9\) (SE \(7.9\)) for the Galerkin form
(Figure~\ref{fig:paired_ladder}).  (\(\dagger\))
...
The full simulation state is \((X_t,S_t,Q_t,I_t,\alpha_t)\), but the reported
control problem restricts the admissible controls to Markovian feedback quotes
\(\delta^\ell(Q_{t-},I_t)\) on the reduced state \((Q_t,I_t)\), eliminating
alpha through \(\alphabar(I_t)\): the RHJB below is the optimality benchmark
within this reduced-information subclass of \(\mathcal A\), not over the full
filtration under which \(\alpha_t\) is observable.  An exploratory three-state
...
the closed-form error itself in the fast-alpha model.  An elementary
...
or tuning constants.  The word \emph{empirical} describes the
...
amplitude because it lies very close to the one-dimensional depth-RMSE optimum
against the RHJB, whereas the profile-consistent value underfits the alpha skew
markedly.  The state-dependent curvature deliberately reads the
...
the Galerkin-cubic form trails it by \(-19.6\pm1.1\)
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.62\linewidth]{regime_fast_alpha_inventory_by_regime.png}
\caption{Regime study inventory by slow regime.  The regime-aware RHJB
policy separates persistent buy- and sell-pressure states more strongly than
the policies that observe only \(I_t\).}
...
level, against \(+4435.0\) (paired s.e.\ \(5.3\)) from observing flow and
...
(\(\approx2283\)), the ordering of the depth figures; with paired standard
errors of the same order as for \(\Delta_R\), these are statistically clear
...
\(\tau\approx850\)--\(1085\) in all runs.  The exact finite-grid GLFT benchmark
...
depths.  Along the imbalance axis the mean is uniform over the \(121\) grid
points (or weighted by the stationary law where stated) and pooled over
...
through the frozen-\(I\) mean \(\alphabar(I)\), whose slope overstates the
exact conditional mean by \(23\%\) (Remark~\ref{rem:frozen_i_robustness});
...
does not observe \(I_t\), so the first ladder step bounds the value of flow
information from above.  Natural next steps are a filtered-regime policy, an
...
\emph{Handbook of Financial Markets: Dynamics and Evolution}, pages 57--156.
...
\emph{Studies in Nonlinear Dynamics \& Econometrics}, 8(3):1--35, 2004.
